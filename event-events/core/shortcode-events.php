...
function displayevents( $atts ) {
	...
	foreach ($allevents as $event) {
		...
		if(get_option('EVNT_links') == "none") { $targetLoc = "_self"; $targetInf = "_self"; }
		elseif(get_option('EVNT_links') == "both") { $targetLoc = "_blank"; $targetInf = "_blank"; }
		elseif(get_option('EVNT_links') == "location") { $targetLoc = "_blank"; $targetInf = "_self"; }
		elseif(get_option('EVNT_links') == "information") { $targetLoc = "_self"; $targetInf = "_blank"; }
		
		// decide if the event location should be a URL
		$evt_loc = NULL;
		if($event['event_loc'] != "" && $event['event_loc_url'] != "" ) {
			$evt_loc = '<div><strong class="event-label">'.__('Where:',EVNT_TEXTDOMAIN).' </strong><a class="location" href="'.httpprefix($event['event_loc_url']).'" target="'.$targetLoc.'">'.$event['event_loc'].'</a></div>';
		} elseif($event['event_loc'] != "") {
			$evt_loc = '<div><strong class="event-label">'.__('Where:',EVNT_TEXTDOMAIN).' </strong>'.$event['event_loc'].'</div>';
		}
		...
		if(isset($atts['moreinfo']) && $atts['moreinfo'] == "no" ) {
			...
			$evt_url = '<div class="info-link"><a class="url" href="'.httpprefix($event['event_url']).'" target="'.$targetInf.'">'.__('More information...',EVNT_TEXTDOMAIN).'</a></div>';		
		}
		
		
		
		// Check if and how the times should be displayed
		$tz = get_option('EVNT_timezone');
		$start_time = '<strong class="event-label">'.__('When:',EVNT_TEXTDOMAIN).' </strong> <abbr class="dtstart" title="'.date('Y-m-d',$event['event_start']).'T'.date('H:i',$event['event_start']).$tz.'">'.$eventtime.'</abbr>';
		$start_notime = '<strong class="event-label">'.__('When:',EVNT_TEXTDOMAIN).' </strong><abbr class="dtstart" title="'.date('Y-m-d',$event['event_start']).'">'.$shorttime.'</abbr>';
