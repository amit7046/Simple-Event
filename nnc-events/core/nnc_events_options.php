<?php
function nnc_events_options() {
	global $wpdb;						
	$table_name = $wpdb->prefix . "nnc_events";

	if (!current_user_can(get_option('NNC_author')))  {
		wp_die( __('You do not have sufficient permissions to manage events.',NNC_TEXTDOMAIN) );
	} // end check authorisation level of the user logged in


// Delete event button
if(isset($_POST['SEsettingsUpdate'])) {
	echo '<div id="message" class="updated fade"><p>'.__('The settings have been saved!',NNC_TEXTDOMAIN).'</p></div>';
}
...
if(isset($_POST['delete'])) {
	   ...
	   if(!$result) {
			echo '<div id="message" class="updated fade"><p>'.__('The event has been deleted!',NNC_TEXTDOMAIN).'</p></div>';
		}
}
...
if(isset($_POST['store_event'])) {
	if($_POST['event']['event_title']) { // New entries at least have to have a title, if none then nothing is written to DB
		...
		if(!$result) {
			echo '<div id="message" class="error fade"><p>'.__('Unfortunately something went wrong...',NNC_TEXTDOMAIN).'</p></div>';
		} else { echo '<div id="message" class="updated fade"><p>'.__('The event was successfully added! Use the shortcode <code>[events]</code> to display the events. Check the <a href="?page=nnc-events&tab=help">Help</a> tab below for more advanced options.',NNC_TEXTDOMAIN).'</p></div>'; }
		// End check  if entry was written successfully to the DB 
	} else { // So if no title was entered for the event then:
		echo '<div id="message" class="error fade"><p>'.__('Please enter the event title',NNC_TEXTDOMAIN).'</p></div>';
...
elseif(isset($_POST['update_event'])) {
	...
	if($result !== false) {
		echo '<div id="message" class="updated fade"><p>'.__('The event was succesfully updated!',NNC_TEXTDOMAIN).'</p></div>';
	} else {
		echo '<div id="message" class="error fade"><p>'.__('There was a problem updating your event!',NNC_TEXTDOMAIN).'</p></div>';
	}
...
<h2 class="nav-tab-wrapper">
    <a href="?page=nnc-events&tab=existing_events" class="nav-tab <?php echo $active_tab == 'existing_events' ? 'nav-tab-active' : ''; ?>"><?php _e('All events',NNC_TEXTDOMAIN);?></a>
    <a href="?page=nnc-events&tab=new_events" class="nav-tab <?php echo $active_tab == 'new_events' ? 'nav-tab-active' : ''; ?>"><?php _e('Add event',NNC_TEXTDOMAIN);?></a>
    <a href="?page=nnc-events&tab=help" class="nav-tab <?php echo $active_tab == 'help' ? 'nav-tab-active' : ''; ?>"><?php _e('Help',NNC_TEXTDOMAIN);?></a>
</h2>
...
<?php } // End of nnc_events_options function 